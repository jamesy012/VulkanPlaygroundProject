...
void VulkanManager::Create(Window *aWindow) {
  ASSERT_IF(NUM_SHADOW_CASCADES <= 4);

  ASSERT_INVALID(_VulkanManager);
  _VulkanManager = this;

  ASSERT_INVALID(gRenderManager);
  ...
  {
    mPresentRenderPass.Create(mDevice, mSwapChainImageFormat,
  ...
  // descriptor
  {
    ...
    vkCreateDescriptorPool(GetDevice(), &poolCreate, GetAllocationCallback(),
                           &mDescriptorPool);
  }

  {
    ...
    if (vkCreateSampler(GetDevice(), &samplerInfo, nullptr, &mDefaultSampler) !=
        VK_SUCCESS) {
    ...
    if (vkCreateSampler(GetDevice(), &samplerInfo, nullptr,
                        &mDefaultMirrorSampler) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture gltfSampler!");
    }
    samplerInfo.addressModeU = samplerInfo.addressModeV =
        samplerInfo.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    if (vkCreateSampler(GetDevice(), &samplerInfo, nullptr,
                        &mDefaultClampedSampler) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture gltfSampler!");
    }
  }
...
void VulkanManager::Destroy() {
  ...
  vkDestroySampler(GetDevice(), mDefaultClampedSampler,
                   GetAllocationCallback());
  vkDestroyDescriptorPool(GetDevice(), mDescriptorPool,
                          GetAllocationCallback());
  ...
  // ImGui
  {
    ...
    vkFreeCommandBuffers(mDevice, mGraphicsCommandPool, mNumSwapChainImages,
                         mImGuiCommandBuffers.data());
    ...
    vkDestroyDescriptorPool(mDevice, mImGuiDescriptorPool,
                            GetAllocationCallback());
  ...
  vkFreeCommandBuffers(mDevice, mGraphicsCommandPool,
  ...
  ASSERT_VALID(_VulkanManager);
  _VulkanManager = nullptr;
}

void VulkanManager::WaitDevice() { vkDeviceWaitIdle(mDevice); }
...
bool VulkanManager::RenderStart(VkCommandBuffer &aBuffer,
                                uint32_t &aFrameIndex) {
  ...
  if (mImagesInFlight[aFrameIndex] != VK_NULL_HANDLE) {
    vkWaitForFences(mDevice, 1, &mImagesInFlight[aFrameIndex], VK_TRUE,
                    UINT64_MAX);
  }
...
void VulkanManager::RenderSubmit(std::vector<VkCommandBuffer> aCommandBuffers) {
  ...
  VkCommandBuffer commandBuffersList[2] = {
      mCommandBuffers[mCurrentImageIndex],
      mImGuiCommandBuffers[mCurrentImageIndex]};
  submitInfo.commandBufferCount =
      sizeof(commandBuffersList) / sizeof(commandBuffersList[0]);
  ...
  if (vkQueueSubmit(mGraphicsQueue.mQueue, 1, &submitInfo,
                    mInFlightFences[mCurrentFrameIndex]) != VK_SUCCESS) {
...
void VulkanManager::DebugSetName(VkObjectType aType, uint64_t aObject,
                                 std::string aName) {
...
void VulkanManager::DebugMarkerStart(VkCommandBuffer aBuffer, std::string aName,
                                     glm::vec4 aColor) {
...
void VulkanManager::DebugMarkerInsert(VkCommandBuffer aBuffer,
                                      std::string aName, glm::vec4 aColor) {
...
const void VulkanManager::BlitRenderTargetToBackBuffer(
    VkCommandBuffer aCommandBuffer, RenderTarget *aRenderTarget) const {
  VulkanManager::Get()->DebugMarkerStart(aCommandBuffer,
                                         "Blit RT to Backbuffer");

  const VkImage presentImage = GetPresentImage(mCurrentFrameIndex);

  SetImageLayout(
      aCommandBuffer, presentImage, VK_IMAGE_ASPECT_COLOR_BIT,
  ...
  vkCmdBlitImage(aCommandBuffer, aRenderTarget->GetColorImage(),
                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, presentImage,
...
bool VulkanManager::CreateInstance() {
  ...
  // nullptr); std::vector<VkExtensionProperties>
  // vkExtensions(vkExtensionCount);
  // vkEnumerateInstanceExtensionProperties(nullptr, &vkExtensionCount,
  // vkExtensions.data()); std::cout << "available extensions:\n";
  //
  ...
  VkResult result =
      vkCreateInstance(&createInfo, GetAllocationCallback(), &mInstance);
  ...
  if (enableValidationLayers) {
    ...
    if (func != nullptr) {
      func(mInstance, &debugCreateInfo, GetAllocationCallback(),
           &mDebugMessenger);
    }
  }

  mDebugMarkerSetObjectName =
      (PFN_vkSetDebugUtilsObjectNameEXT)vkGetInstanceProcAddr(
          mInstance, "vkSetDebugUtilsObjectNameEXT");
  mDebugMarkerBegin = (PFN_vkCmdBeginDebugUtilsLabelEXT)vkGetInstanceProcAddr(
      mInstance, "vkCmdBeginDebugUtilsLabelEXT");
  mDebugMarkerEnd = (PFN_vkCmdEndDebugUtilsLabelEXT)vkGetInstanceProcAddr(
      mInstance, "vkCmdEndDebugUtilsLabelEXT");
  mDebugMarkerInsert = (PFN_vkCmdInsertDebugUtilsLabelEXT)vkGetInstanceProcAddr(
      mInstance, "vkCmdInsertDebugUtilsLabelEXT");
...
QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice aDevice,
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(aDevice, &queueFamilyCount,
                                           queueFamilies.data());
...
SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice aDevice,
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(aDevice, aSurface,
                                            &details.capabilities);

  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(aDevice, aSurface, &formatCount,
                                       nullptr);
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(aDevice, aSurface,
                                            &presentModeCount, nullptr);

  if (presentModeCount != 0) {
    details.presentModes.resize(presentModeCount);
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        aDevice, aSurface, &presentModeCount, details.presentModes.data());
  }
...
bool CheckDeviceExpensionSupport(VkPhysicalDevice aDevice) {
  ...
  vkEnumerateDeviceExtensionProperties(aDevice, nullptr, &extensionCount,
                                       nullptr);
...
bool IsDeviceSuitable(VkPhysicalDevice aDevice, VkSurfaceKHR aSurface) {
  ...
  if (extensionsSupported) {
    SwapChainSupportDetails swapChainSupport =
        querySwapChainSupport(aDevice, aSurface);
    swapChainAdequte = !swapChainSupport.formats.empty() &&
  ...
  return indices.graphicsFamily.has_value() && extensionsSupported &&
         swapChainAdequte && supportedFeatures.samplerAnisotropy;
}

bool VulkanManager::CreateDevice() {
  ASSERT_VULKAN_HANDLE(mInstance);
  // physical device
  {
...
bool VulkanManager::CreateSwapchain() {
  ...
  vkGetSwapchainImagesKHR(mDevice, mSwapChain, &mNumSwapChainImages,
  ...
  // create Views
  {
    ...
    for (uint32_t i = 0; i < mNumSwapChainImages; i++) {
      ...
      vkCreateImageView(mDevice, &imageViewInfo, GetAllocationCallback(),
...
bool VulkanManager::CreateCommandPoolBuffers() {
  // pool
  {
    ...
    if (vkCreateCommandPool(mDevice, &poolInfo, nullptr,
                            &mGraphicsCommandPool) != VK_SUCCESS) {
      ASSERT_RET_FALSE("failed to create command pool!");
    }
    DebugSetObjName(VK_OBJECT_TYPE_COMMAND_POOL, mGraphicsCommandPool,
                    "Main Command Pool");
  }
  // buffers
  {
    ...
    for (size_t i = 0; i < mCommandBuffers.size(); i++) {
      DebugSetObjName(VK_OBJECT_TYPE_COMMAND_BUFFER, mCommandBuffers[i],
                      "Main CommandBuffer Frame:" + std::to_string(i));
    }
  }
...
bool VulkanManager::CreateSyncObjects() {
  ...
  for (size_t i = 0; i < mNumSwapChainImages; i++) {
    ...
    // setObjName(VK_OBJECT_TYPE_SEMAPHORE,
    // (uint64_t)mImageAvailableSemaphores[i], "imageAvailableSemaphores Frame:"
    // + std::to_string(i)); setObjName(VK_OBJECT_TYPE_SEMAPHORE,
    // (uint64_t)mRenderFinishedSemaphores[i], "renderFinishedSemaphores Frame:"
    // + std::to_string(i)); setObjName(VK_OBJECT_TYPE_FENCE,
    // (uint64_t)mInFlightFences[i], "inFlightFences Frame:" +
    // std::to_string(i));
  }
  return true;
}
...
bool VulkanManager::CreateImGui() {
  // Create Descriptor Pool
  {
    ...
    VkResult result = vkCreateDescriptorPool(mDevice, &pool_info, nullptr,
                                             &mImGuiDescriptorPool);
    ASSERT_VULKAN_SUCCESS_RET_FALSE(result);
    // setObjName(VK_OBJECT_TYPE_DESCRIPTOR_POOL, (uint64_t)imguiDescriptorPool,
    // "ImGUI Descritor Pool");
  }
  ...
  // Create the Render Pass
  {
    ...
    VkResult result =
        vkCreateRenderPass(mDevice, &info, nullptr, &mImGuiRenderPass);
    ASSERT_VULKAN_SUCCESS_RET_FALSE(result);
    // setObjName(VK_OBJECT_TYPE_RENDER_PASS, (uint64_t)wd->RenderPass, "ImGui
    // Render Pass");
    ...
    // create one with: ImGui_ImplVulkan_CreatePipeline(device, allocator,
    // VK_NULL_HANDLE, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline,
    // g_Subpass);
  }
  ...
  if (vkAllocateCommandBuffers(mDevice, &allocInfo,
                               mImGuiCommandBuffers.data()) != VK_SUCCESS) {
    ASSERT_RET_FALSE("failed to allocate command buffers!");
  }
  for (size_t i = 0; i < mImGuiCommandBuffers.size(); i++) {
    DebugSetObjName(VK_OBJECT_TYPE_COMMAND_BUFFER, mImGuiCommandBuffers[i],
                    "ImGui Command Buffers Buffers Frame:" + std::to_string(i));
  }
...
bool VulkanManager::CreateImGuiSizeDependent() {
  ...
  // Create Framebuffer
  {
    ...
    for (uint32_t i = 0; i < mNumSwapChainImages; i++) {
      ...
      VkResult result =
          vkCreateFramebuffer(mDevice, &info, nullptr, &mImGuiFramebuffer[i]);
      ASSERT_VULKAN_SUCCESS_RET_FALSE(result);
      // setObjName(VK_OBJECT_TYPE_FRAMEBUFFER, (uint64_t)imguiFramebuffers[i],
      // "ImGui Frame Buffers Frame:" + std::to_string(i));
    }
  }
  return true;
}
...
void VulkanManager::RenderImGui() {
  ...
  ImDrawData *draw_data = ImGui::GetDrawData();
  {
    VkResult result;
    {
      ...
      result = vkBeginCommandBuffer(mImGuiCommandBuffers[mCurrentImageIndex],
                                    &bufferBeginInfo);
      ASSERT_VULKAN_SUCCESS(result);
      VulkanManager::Get()->DebugMarkerStart(
          mImGuiCommandBuffers[mCurrentImageIndex], "ImGui Render");
      ...
      renderBeginInfo.framebuffer =
          mPresentFramebuffer[mCurrentImageIndex].GetFramebuffer();
#endif
      // renderBeginInfo.framebuffer =
      // mPresentFramebuffer[mCurrentImageIndex].GetFramebuffer();
      ...
      renderBeginInfo.pClearValues = clearColor.data();
#endif
      vkCmdBeginRenderPass(mImGuiCommandBuffers[mCurrentImageIndex],
                           &renderBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
      ...
      VulkanManager::Get()->DebugMarkerEnd(
          mImGuiCommandBuffers[mCurrentImageIndex]);
...
#if IMGUI_OWN_FRAMEBUFFER
    vkDestroyFramebuffer(mDevice, mImGuiFramebuffer[i],
                         GetAllocationCallback());
#endif
...
void VulkanManager::CreateSizeDependent() {
  ...
  if (mPresentFramebuffer.size() == 0) {
    ...
    for (uint32_t i = 0; i < mNumSwapChainImages; i++) {
      ...
      mPresentFramebuffer[i].Create(mDevice, mSwapChainExtent,
                                    &mPresentRenderPass, view);
    }
  }
